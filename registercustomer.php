...
// (original PHP processing continues below)
//
?>
<?php
//function to clean input but not validate type and content
...
//save the customer data if the error flag is still clear
    if ($error == 0) {
      ...
      if ($stmt) {
        // bind the email (was incorrectly using $username which caused NULL email error)
        mysqli_stmt_bind_param($stmt, 'ssss', $firstname, $lastname, $email, $password);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
        echo "<h2>customer saved</h2>";
      } else {
        echo "<h2>Error preparing statement</h2>".PHP_EOL;
      }
    } else {
...
?>
    ...
    <h2><a href='listcustomers.php'>[Return to the Customer listing]</a><a href='/bnb_2/'>[Return to the main page]</a></h2>
